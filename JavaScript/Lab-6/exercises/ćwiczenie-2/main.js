...
window.onload = function () {
    ...
    for (let i = 0; i < digitButtons.length; i++) {
        digitButtons[i].addEventListener('click', function (event){
            digitBtnListener(event);
        });
    }

    for (let i = 0; i < operatorButtons.length; i++) {
        operatorButtons[i].addEventListener('click', function (event){
            operatorBtnLister(event);
        });
    }

    displayClearBtn.addEventListener('click', function (event){
        displayClearBtnListener(event);
    });
    ...
    document.addEventListener('keyup', function (event){
        ...
        mainDisplay.addDigit(event.key);
    });
